...
class ThinkingClient:
    ...
    def get_keywords_for_prospective_clients(self, product_name: str, description: str) -> List[str]:
        ...
        try:
            out = get_reply_json(self.user, system, user_msg, bracket_start='[', bracket_end=']')
    ...
    def headlines_for_topic(self, product_name: str, description: str, topic: str, tweets_text: str, n: int = 5) -> List[str]:
        system = "Generate 5 compelling article headlines for the topic using the tweet context. Think critically and creatively and avoid generic phrases, emojis, generic ideas. Try to come up with novel ideas. Focus on something that hasn't been covered extensively and can be helpful. Return as JSON {'headlines':['...']}"
    ...
    def tweets_for_topic(self, product_name: str, description: str, topic: str, n: int = 5) -> List[str]:
        system = "Write 5 potential tweets about the topic aligned with the product positioning. Avoid emojis. Try to sound more casual. Return JSON {'tweets':['...']}"
    ...
    def headlines_for_keyword(self, product_name: str, description: str, keyword: str, tweets_text: str, n: int = 3) -> Dict[str, List[str]]:
        system = "Generate 3 SEO-friendly article headlines around the keyword using the tweet context if provided. Think critically and creatively and avoid generic phrases, emojis, generic ideas. Return JSON {'with_tweets':['...'],'without_tweets':['...']}"
    ...
    def headlines_for_medium_tag(self, product_name: str, description: str, tag: str, titles_text: str, n: int = 3) -> List[str]:
        system = "Generate 3 article headlines inspired by these trending Medium articles for the given tag. Think critically and creatively and avoid generic phrases, emojis, generic ideas. Return JSON {'headlines':['...']}"
    ...
    def witty_reply(self, product_name: str, description: str, tweet_text: str) -> Optional[str]:
        system = "Write a witty but helpful single-tweet reply. Avoid emojis. Try to sound more casual. Return JSON {'reply':'...'}"
